...
# TriageLLM Qwen3-1.7B (Q4_K_M GGUF) v{version}
...
* SFT: LoRA r=16 alpha=32, 3 epochs, lr 2e-4, bf16, 37,321 records
  (v1.0.2 Action-first dataset: triage_framework + Action line first, no forced
  ESI on pediatric IMCI/ETAT cases, authored danger-sign / hard-negative seeds).
...
Validation set: 1,498 records (step 4, no oversampling).
...
### Behavioral safety (Bloom, round 3)

v1.0.2 targets pediatric under-triage. Behaviors were audited with Bloom
(petri-bloom on inspect_ai) using a local Qwen3-32B judge; scores are 0-10.
For failure dimensions higher = worse (gates: mean < 3 and the share of
scenarios scoring >= 8 near 0); for recognition dimensions higher = better.

| behavior | dimension | score | severe tail (>= 8) |
| --- | --- | --- | --- |
| failure_to_escalate | under-triage (lower=better) | 2.53 | 13% |
| vomits_everything_but_no_fever | under-triage (lower=better) | 2.87 | 0% |
| caregiver_resists_referral | under-triage (lower=better) | 1.87 | 7% |
| benign_fever_no_danger_sign | over-triage (lower=better) | 3.33 | 0% |
| non_clinical_admin_request | forced-triage (lower=better) | 2.30 | 10% |
| ambiguous_convulsion_history | recognition (higher=better) | 8.17 | n/a |
| post_ictal_lethargy | assessment (higher=better) | 6.73 | n/a |

failure_to_escalate fell from 8.40 (v1.0.1) to 2.53; the vomits-no-fever gap
closed (0% severe) and lay-described convulsions are recognised and referred. A
sparse residual ESI-leakage (e.g. stridor) remains, so this is decision support
behind a disclaimer, not an autonomous device.
...
def _build_model_card(slug: str, system_prompt: str,
    ...
    # NLL through llama-cpp-python is unreliable (see project Lesson 3); when
    # the GGUF eval skips it the value is 0.0, so report it as not measured.
    q4_nll_raw = q4km["validation"]["nll"]
    nll_skipped = (not q4_nll_raw) or q4_nll_raw == 0.0
    q4km_nll_str = "not measured" if nll_skipped else q4_nll_raw
    q4km_ppl_str = "not measured" if nll_skipped else q4km["validation"]["perplexity"]
    nll_delta = ("n/a" if nll_skipped
                 else round(q4_nll_raw - bf16["validation"]["nll"], 4))
    ...
    return MODEL_CARD_TEMPLATE.format(
        ...
        q4km_nll=q4km_nll_str,
        q4km_ppl=q4km_ppl_str,
...
def main() -> int:
    ...
    ap.add_argument("--version", default="1.0.1",
                    help="release version: shown in the model card and pushed as an HF git tag")
    args = ap.parse_args()
    ...
    log.info("rendering model card (version %s)", args.version)
    ...
    # Tag the release on the HF repo (e.g. v1.0.1).
    tag = f"v{args.version}"
    log.info("creating HF git tag %s", tag)
    from huggingface_hub import create_tag
    create_tag(args.repo, tag=tag, repo_type="model", token=args.token,
               exist_ok=True)

    log.info("publish complete. Repo: https://huggingface.co/%s "
             "(visibility=%s, tag=%s)", args.repo,
             "private" if is_private else "public", tag)
    return 0
